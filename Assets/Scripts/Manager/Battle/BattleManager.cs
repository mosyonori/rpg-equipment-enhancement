...
public class BattleManager : MonoBehaviour
{
    ...
    private void LogPlayerEquipmentDetails(UserSaveData userData)
    {
        ...
        // 装備中のアイテム詳細確認（HomeManager の UpdateEquipmentSummary相当）
        if (userData.equipments != null)
        {
            ...
            foreach (var item in equippedItems)
            {
                ...
                if (masterData != null)
                {
                    ...
                    // 戦闘力計算（UserDataUtility の CalculateTotalPowerと同じロジック）
    ...
    private void InitializeBattle()
    {
        ...
        try
        {
            ...
            // BattleCalculationManager に も BattleDataManager の参照を設定
    ...
    /// <summary>
    /// 📝大幅修正: 戦闘参加キャラクター作成（BattleMonsterManager使用版）
    /// </summary>
    private void CreateBattleCharacters()
    {
        ...
        // 📝重要修正: 敵モンスター作成をBattleMonsterManagerに委譲
        Log($"敵モンスター作成開始: {currentBattleSetup.spawnMonsterIds.Count}体");

        try
        {
            // BattleMonsterManagerを使用してモンスターを生成
            var generatedMonsters = battleMonsterManager.GenerateBattleMonsters(
                currentBattleSetup.spawnMonsterIds
            );

            // 生成されたモンスターを戦闘キャラクターリストに追加
            allCharacters.AddRange(generatedMonsters);

            Log($"敵モンスター作成完了: {generatedMonsters.Count}体");
            foreach (var monster in generatedMonsters)
            {
                Log($"生成完了: {monster.displayName} (characterID: {monster.characterId}, instanceID: {monster.instanceId})");
            }
        }
        catch (Exception e)
        {
            LogError($"敵モンスター作成エラー: {e.Message}");
            LogError($"スタックトレース: {e.StackTrace}");

            // フォールバック: 従来の方法でモンスターを作成
            Log("フォールバック: 従来の方法でモンスター作成を試行");
    ...
    /// <summary>
    /// 📝追加: フォールバック用の従来モンスター作成メソッド
    /// </summary>
    private void CreateMonstersAsFallback()
    {
        Log("フォールバックモンスター作成開始");
        ...
        foreach (var monsterId in currentBattleSetup.spawnMonsterIds)
        {
            ...
            {
                LogError($"フォールバックモンスター作成エラー (ID={monsterId}): {e.Message}");
                var fallbackMonster = CreateFallbackMonster(monsterId);
                allCharacters.Add(fallbackMonster);
            }
        }

        Log("フォールバックモンスター作成完了");
    }
    ...
    /// <summary>
    /// 修正: プレイヤーの装備ステータス計算（HomeManager の EquipmentSummaryData.CreateFromSaveDataパターンを参考）
    /// </summary>
